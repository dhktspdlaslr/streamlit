...
with st.expander('이 앱에 대하여'):
  ...
  st.image('https://th.bing.com/th/id/OIP.hK5q86yFD58YrDXZUOyW-AHaE8?rs=1&pid=ImgDetMain', width=250)

st.sidebar.header('입력')
user_name = st.sidebar.text_input('당신의 이름은 무엇인가요?')
user_emoji = st.sidebar.selectbox('이모티콘 선택', ['', '😄', '😆', '😊', '😍', '😴', '😕', '😱'])
user_food = st.sidebar.selectbox('가장 좋아하는 음식은?', ['', 'Bibimbap', 'Burrito', 'Lasagna', 'Hamburger', 'Pizza'])
